...
class M_fasilitaskamar extends CI_model {
	
	public function getall($where){
		$this->db->select('*');
		$this->db->from('tb_fasilitas_kamar');
		$this->db->join('tb_kost', 'tb_kost.id_kost = tb_fasilitas_kamar.id_kost');
		$this->db->where($where);
		return $this->db->get();
		}
	...
	public function cek($where){
		...
		$this->db->join('tb_fasilitas_kamar', 'tb_kost.id_kost = tb_fasilitas_kamar.id_kost');
		$this->db->where($where);
		return $this->db->get();
	}

	public function simpan($data){
		$this->db->insert('tb_fasilitas_kamar', $data);
	}
	...
	public function edit($data, $where){
		$this->db->where($where);
		$this->db->update('tb_fasilitas_kamar', $data);
	}

	public function hapus($where){
		$this->db->where($where);
		$this->db->delete('tb_fasilitas_kamar');
	}
